...
<html>
  ...
  <body>
    <div class="row canvas">
      <header>
        <h1 class="ye-title">My Local Sites</h1>
        <nav>
          <ul>
  ...
  <?php foreach ( $dir as $d ) {
          ...
          foreach( glob( $d ) as $file )  {
            ...
            $siteroot = sprintf( 'http://%1$s/%2$s', $tld, $project );
            ...
            printf( '<a class="site" href="%1$s">%2$s</a>', $siteroot, ucwords($displayname) );

            // Display an icon with a link to the admin area
            $adminurl = '';

            // Check if the site appears to be WordPress
